...
Name of File: LineNumbers.cpp
Description: Asks for an input an output file and prints to the output file 
...
int main() {
    std::cout << "Please enter the name of the .txt file you want to read:" << std::endl;
    std::getline(std::cin, inputFile);

    std::ifstream inFile(inputFile);
    std::string text;

    std::cout << "Please enter the name of your .txt file you want to output to: " << std::endl;
    std::getline(std::cin, outputFile);

    std::ofstream outFile(outputFile);

    while (std::getline(inFile, text)) {
        outFile << lineNumber << ". " << text << "\n";
        lineNumber += 1;
    }
